...
const Navbardetail = (props:Props) => {
  ...
  const { cart } = state;
  useEffect(()=>{
    setCartcount(cart.cartItems.reduce((a: any, c: any) => a + c.qty, 0) )
  },[cart.cartItems])
  ...
  return (
    <div className="flex flex-col gap-5 left-0 sticky top-0 md:sticky z-[999] bg-white  items-center pt-[10px]   shadow-md  ">
      ...
      <div className="flex  lg:hidden gap-[1rem] pt-5">
        ...
        <Link href={'/Cart'}><BiCart className="   w-[30px] h-[30px] ml-[20px] " /></Link>
        {cartcount > 0 && (
      ...
      <div className="flex justufy-start w-screen">
        <select  name="" id="">
          <option value="none" defaultValue={'All Categories'} className="clear">
            All Categories
          </option>
